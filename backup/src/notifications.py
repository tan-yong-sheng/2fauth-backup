...
class NotificationManager:
    ...
    def notify_backup_success(
        ...
        if self.smtp_host:
            subject = f"[2FAuth Backup] Success - {backup_id}"
            body = f"""2FAuth Backup Completed Successfully
...
"""
    ...
    def notify_backup_failure(
        ...
        if self.smtp_host:
            subject = f"[2FAuth Backup] FAILED - {backup_id}"
            body = f"""2FAuth Backup FAILED
...
"""
    ...
    def notify_restore_success(self, backup_id: str, restore_path: str):
        ...
        if self.smtp_host:
            subject = f"[2FAuth Backup] Restore Success - {backup_id}"
            body = f"""2FAuth Restore Completed Successfully
...
"""
    ...
    def notify_restore_failure(self, backup_id: str, error_message: str):
        ...
        if self.smtp_host:
            subject = f"[2FAuth Backup] Restore FAILED - {backup_id}"
            body = f"""2FAuth Restore FAILED
